...
def diversity(pw_distances: np.ndarray, gamma: float = 0.8) -> float:
    """Diversity, i.e. ratio between number of pairwise distances above
    threshold and total number of distances.
...
def coverage(summarized_adjacency: sparse.csr_matrix) -> float:
    """Node coverage of summarized graph, i.e. ratio between number of nodes
    in summarized graph and number of nodes in original graph.
    ...
    n_nodes = len(np.flatnonzero(summarized_adjacency.dot(
...
def expressiveness(summarized_adjacency: sparse.csr_matrix,
                   pw_distances: np.ndarray, gamma: float,
